...
pub fn main() {
	...
	let mut map = maps.next().to_owned().expect("could not obtain first map").to_owned();
	...
	while window.is_open() && !window.is_key_down(Key::Escape) && !exit{
		...
		for t in keys {
			...
			if dir.is_some() {
				if player.mv(dir.unwrap(), &mut map) {
					
					let map_option = maps.next();
					if map_option.is_some() {
						map = map_option.unwrap().to_owned();
						player = stuff::Player::new(2.0, 2.0, angle, false);
					} else {
						exit = true;
					}
				}
